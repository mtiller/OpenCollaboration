...
package PolyBased_T "EoS for a liquid, density independent of p, properties defined by polynomials"
extends ORNL_AdvSMR.Media.EquationsOfState.Interfaces.Liquids(final
    analyticInverseTfromh=(not data.npol_Cp > 2), final CompressibilityType=
      Compressibility.TemperatureDependent);
...
redeclare function extends dudp_pTX
  algorithm
  dudp := -1/max(Poly.evaluate(data.polyDensity, T), limits.DMIN);
  end dudp_pTX;
...
<p>The constant <code>CompressibilityType</code> is forced to <code>TemperatureDependent.</code>
<p><ul>
<li><code>TemperatureDependent: </code>Density is independent of pressure. Specific volume is given by v(</ul></p>
