...
package PolyBased_pT "EoS for a fully compressible liquid with properties defined by polynomials"
extends ORNL_AdvSMR.Media.EquationsOfState.Interfaces.Liquids(final
    analyticInverseTfromh=(not data.npol_Cp > 2), final CompressibilityType=
      Compressibility.FullyCompressible);
...
constant Modelica.SIunits.SpecificVolume v0=1/max(Poly.evaluate(data.polyDensity,
    T0), limits.DMIN);
...
redeclare function extends v_pTX
  algorithm
  v := (data.reference_p - p)*data.kappa_const*v0 + 1/max(Poly.evaluate(data.polyDensity,
    T), limits.DMIN);
...
redeclare function extends s_pTX
  algorithm
  s := data.reference_s - (p - data.reference_p)*(Poly.derivativeValue(data.polyDensity,
    T)/(max(Poly.evaluate(data.polyDensity, T), limits.DMIN)*max(Poly.evaluate(
    data.polyDensity, T), limits.DMIN))) + Poly.integralValue(
...
redeclare function extends dddT_pTX
  algorithm
  dddT := Poly.derivativeValue(data.polyDensity, T)/(v_pTX(
      p,
      T,
      X)*v_pTX(
      p,
...
redeclare function extends dudp_pTX
  algorithm
  dudp := (2*p - data.reference_p)*data.kappa_const*v0 - 1/max(Poly.evaluate(
    data.polyDensity, T), limits.DMIN);
  end dudp_pTX;
...
redeclare function extends dudT_pTX
  ...
  dudT := cp_pTX(
      p,
      T,
      X) + p*Poly.derivativeValue(data.polyDensity, T)/(max(Poly.evaluate(data.polyDensity,
...
<p>The constant <code>CompressibilityType</code> is forced to <code>FullyCompressible.</code>
<p>The case described above. v0 is defined as 1/rho(T0,p0) and calculated by evaluating the polynomial. <p>Note that the compressibility factors beta and kappa can always be obtained from the respective function <p><h4><font color=\"#008000\">Analytic derivatives</font></h4></p>
