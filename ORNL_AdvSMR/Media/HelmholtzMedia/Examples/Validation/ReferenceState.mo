...
protected
  ...
  final constant Medium.Temperature T_IIR=273.15;
  // 0°C;
  final constant Medium.Temperature T_ASHRAE=233.15;
  // -40°C;
  final constant Medium.AbsolutePressure p_NBP=101325;
  // 1.01325 bar = 1 atm
...
algorithm
  ...
  Modelica.Utilities.Streams.print(String(Medium.helmholtzCoefficients.idealPower[
    1, 1], significantDigits=15) + ";" + String(s_ref, significantDigits=15) +
    ";" + String(Medium.helmholtzCoefficients.idealPower[2, 1],
    significantDigits=15) + ";" + String(h_ref, significantDigits=15) + ";",
    fileName);
...
</html>"), experiment(NumberOfIntervals=1));
end ReferenceState;
