...
algorithm
  ...
  if (state.phase == 2) then
    assert(p <= p_crit,
      "setState_pdX_error: pressure is higher than critical pressure");
    sat := setSat_p(p=p);
    assert(d <= sat.liq.d,
      "setState_pdX_error: density is higher than saturated liquid density: this is single phase liquid");
    assert(d >= sat.vap.d,
      "setState_pdX_error: density is lower than saturated vapor density: this is single phase vapor");
  else
    if (p < p_crit) and (p > p_trip) then
      ...
      if (p > 0.98*p_crit) or (p < 300*p_trip) then
        ...
        if ((d < sat.liq.d + abs(0.05*sat.liq.d)) and (d > sat.vap.d - abs(0.05
            *sat.vap.d))) then
      ...
      if (d > sat.liq.d) then
        ...
        // look at isobars in T,d-Diagram !!
        T_max := 1.02*sat.Tsat;
        T_iter := 1.1*T_min;
  ...
  if (state.phase == 2) then
    ...
    assert(
      (RES_min*RES_max < 0),
      "setState_pd: T_min=" + String(T_min) + " and T_max=" + String(T_max) +
        " did not bracket the root, input was p=" + String(p) + " and d=" +
        String(d),
    ...
    assert(iter < iter_max, "setState_pdX did not converge (RES_p=" + String(
      RES_p) + "), input was p=" + String(p) + " and d=" + String(d));
