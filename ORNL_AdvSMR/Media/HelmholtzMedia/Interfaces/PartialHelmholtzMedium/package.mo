within ORNL_AdvSMR.Media.HelmholtzMedia.Interfaces;
...
import ORNL_AdvSMR.Media.HelmholtzMedia.Interfaces.PartialHelmholtzMedium.Types
  .*;
...
constant Transport.ThermalConductivityCoefficients
  thermalConductivityCoefficients;
...
redeclare model extends BaseProperties
  ...
  state.s = s;
  state.u = u;
  state.phase = if ((p < fluidConstants[1].criticalPressure) and (p >
    fluidConstants[1].triplePointPressure) and (h > sat.liq.h) and (h < sat.vap.h))
     then 2 else 1;
...
protected
  ...
  Real gamma(
    min=0,
    max=1) = 1 "convergence speed, default=1";
  Integer iter=0;
  ...
  if ((T >= T_trip) and (T < T_crit)) then
    ...
    K_liq := delta_liq*EoS.f_rd(tau=tau, delta=delta_liq) + EoS.f_r(tau=tau,
      delta=delta_liq) + log(delta_liq);
    K_vap := delta_vap*EoS.f_rd(tau=tau, delta=delta_vap) + EoS.f_r(tau=tau,
    ...
    while (abs(Delta_J) + abs(Delta_K) > tolerance) loop
      ...
      // calculate gradients
      J_liq_delta := 1 + 2*delta_liq*EoS.f_rd(tau=tau, delta=delta_liq) +
        delta_liq^2*EoS.f_rdd(tau=tau, delta=delta_liq);
      J_vap_delta := 1 + 2*delta_vap*EoS.f_rd(tau=tau, delta=delta_vap) +
        delta_vap^2*EoS.f_rdd(tau=tau, delta=delta_vap);
      K_liq_delta := 2*EoS.f_rd(tau=tau, delta=delta_liq) + delta_liq*EoS.f_rdd(
        tau=tau, delta=delta_liq) + 1/delta_liq;
      K_vap_delta := 2*EoS.f_rd(tau=tau, delta=delta_vap) + delta_vap*EoS.f_rdd(
        tau=tau, delta=delta_vap) + 1/delta_vap;
      ...
      delta_liq := delta_liq + gamma/det*((K_vap - K_liq)*J_vap_delta - (J_vap
         - J_liq)*K_vap_delta);
      delta_vap := delta_vap + gamma/det*((K_vap - K_liq)*J_liq_delta - (J_vap
         - J_liq)*K_liq_delta);
...
protected
  ...
  if ((p > p_trip) and (p < p_crit)) then
    ...
    fl := EoS.setHelmholtzDerivsSecond(
        d=sat.liq.d,
        T=sat.Tsat,
        phase=1);
    fv := EoS.setHelmholtzDerivsSecond(
        d=sat.vap.d,
        T=sat.Tsat,
        phase=1);
    ...
    while (RSS > tolerance) and (iter < iter_max) loop
      ...
      Jacobian := [+EoS.dpdT(fl), +0, +EoS.dpTd(fl); +0, +EoS.dpdT(fv), +
        EoS.dpTd(fv); +EoS.dgdT(fl), -EoS.dgdT(fv), EoS.dgTd(fl) - EoS.dgTd(fv)];
    ...
    assert(iter < iter_max, "setSat_p did not converge, input was p=" + String(
      p) + "; the remaining residuals are RES_pl=" + String(RES[1]) +
      " and RES_pv=" + String(RES[2]) + " and RES_g=" + String(RES[3]));
...
protected
  ...
  if (state.phase == 0) then
    ...
    if ((T >= T_trip) and (T < T_crit)) then
      ...
      if ((d > 1.05*Ancillary.bubbleDensity_T(T=T)) or (d < 0.98*
          Ancillary.dewDensity_T(T=T))) then
        state.phase := 1;
      else
  ...
  elseif (state.phase == 2) then
    assert(T <= T_crit,
      "setState_dTX_error: Temperature is higher than critical temperature");
    sat := setSat_T(T=T);
    assert(d >= sat.vap.d,
      "setState_dTX_error: density is lower than saturated vapor density: this is single phase vapor");
    assert(d <= sat.liq.d,
      "setState_dTX_error: density is higher than saturated liquid density: this is single phase liquid");
  end if;
...
protected
  ...
  assert(phase <> 2,
    "setState_pTX_error: pressure and temperature are not independent variables in two-phase state");
  state.phase := 1;
  ...
  if (T < T_crit) then
    ...
    elseif (p < sat.psat) then
      ...
      // d_iter := p/(R*T);
      d_iter := Ancillary.density_pT_Soave(
          T=T,
          p=p,
          psat=sat.psat);
    else
      ...
      assert(p <> sat.psat,
        "setState_pTX_error: pressure equals saturation pressure");
    end if;
  else
  ...
  assert(iter < iter_max, "setState_pTX did not converge, input was p=" +
    String(p) + " and T=" + String(T));
...
protected
  ...
  if (state.phase == 2) then
    assert(p <= p_crit,
      "setState_phX_error: pressure is higher than critical pressure");
    sat := setSat_p(p=p);
    assert(h >= sat.liq.h,
      "setState_phX_error: enthalpy is lower than saturated liquid enthalpy: this is single phase liquid");
    assert(h <= sat.vap.h,
      "setState_phX_error: enthalpy is higher than saturated vapor enthalpy: this is single phase vapor");
  else
    if (p < p_crit) then
  ...
  if (state.phase == 2) then
    ...
    while ((RSS > tolerance) and (iter < iter_max)) loop
      ...
      while useLineSearch and (lambda >= lambda_min) and not (RSS <= (RSS_old
           + alpha*lambda*slope)) loop
        ...
        if (iterLineSearch < 2) then
          ...
          b := (-lambda_ls*rhs1/lambda^2 + lambda*rhs2/lambda_ls^2)/(lambda -
            lambda_ls);
          if (a == 0) then
            ...
            lambda_temp := if (lambda_temp > 0.5*lambda) then 0.5*lambda else
              lambda_temp;
          end if;
        end if;
...
protected
  ...
  if (state.phase == 2) then
    assert(p <= p_crit,
      "setState_psX_error: pressure is higher than critical pressure");
    sat := setSat_p(p=p);
    assert(s >= sat.liq.s,
      "setState_psX_error: entropy is lower than saturated liquid entropy: this is single phase liquid");
    assert(s <= sat.vap.s,
      "setState_psX_error: entropy is higher than saturated vapor entropy: this is single phase vapor");
  else
    if (p <= p_crit) then
      // two-phase possible, check region
      if (p > 0.98*p_crit) or (p < 300*p_trip) then
        ...
        if ((s > sat.liq.s - abs(0.05*sat.liq.s)) and (s < sat.vap.s + abs(0.05
            *sat.vap.s))) then
          // Modelica.Utilities.Streams.print("two-phase state or close to it, get saturation properties from EoS", "printlog.txt");
          sat := setSat_p(p=p);
        end if;
      ...
      if (s <= s_crit) then
        ...
        if (s < state_ref.s) then
          T_min := max(fluidLimits.TMIN*0.99,
            Ancillary.saturationTemperature_s_liq(s=s)*0.95);
          T_max := T_crit;
          T_iter := 0.95*T_crit;
  ...
  if (state.phase == 2) then
    ...
    while ((RSS > tolerance) and (iter < iter_max)) loop
      ...
      assert(
          slope < 0,
          "roundoff problem, input was p=" + String(p) + " and s=" + String(s),
          level=AssertionLevel.warning);
    ...
    assert(iter < iter_max, "setState_psX did not converge, input was p=" +
      String(p) + " and s=" + String(s) + ", remaining residual is RES_p=" +
      String(RES[1]) + " and RES_s=" + String(RES[2]));
...
protected
  ...
  assert(
      state.T >= T_trip,
      "vapourQuality warning: Temperature is lower than triple-point temperature",
      level=AssertionLevel.warning);

  assert(
      state.T <= T_crit,
      "vapourQuality warning: Temperature is higher than critical temperature",
      level=AssertionLevel.warning);
...
protected
  ...
  elseif (state.phase == 2) then
    assert(
        false,
        "specificHeatCapacityCp warning: in the two-phase region cp is infinite",
        level=AssertionLevel.warning);
...
protected
  ...
  elseif (state.phase == 2) then
    ...
    duT_liq := EoS.duTd(fl) - EoS.dudT(fl)*EoS.dpTd(fl)/EoS.dpdT(fl) + EoS.dudT(
      fl)/EoS.dpdT(fl)*dpT;
    duT_vap := EoS.duTd(fv) - EoS.dudT(fv)*EoS.dpTd(fv)/EoS.dpdT(fv) + EoS.dudT(
      fv)/EoS.dpdT(fv)*dpT;
...
protected
  ...
  assert(state.phase <> 2,
    "velocityOfSound error: property not defined in two-phase region");
  f := EoS.setHelmholtzDerivsSecond(
      T=state.T,
      d=state.d,
      phase=1);
...
protected
  ...
  elseif (state.phase == 2) then
    assert(
        false,
        "isobaricExpansionCoefficient warning: in the two-phase region beta is zero",
        level=AssertionLevel.warning);
...
protected
  ...
  elseif (state.phase == 2) then
    assert(
        false,
        "isothermalCompressibility warning: in the two-phase region kappa is infinite",
        level=AssertionLevel.warning);
...
protected
  ...
  elseif (state.phase == 2) then
    assert(
        false,
        "isentropicExponent warning: in the two-phase region gamma is infinite",
        level=AssertionLevel.warning);
...
protected
  ...
  assert(
      state.phase <> 2,
      "dynamicViscosity warning: property not defined in two-phase region",
      level=AssertionLevel.warning);
...
redeclare replaceable function extends thermalConductivity
  ...
  assert(
      state.phase <> 2,
      "thermalConductivity warning: property not defined in two-phase region",
      level=AssertionLevel.warning);
...
protected
  ...
  Real[size(surfaceTensionCoefficients.coeffs, 1)] a=surfaceTensionCoefficients.coeffs[
      :, 1];
  Real[size(surfaceTensionCoefficients.coeffs, 1)] n=surfaceTensionCoefficients.coeffs[
      :, 2];
  Real X "reduced temperature difference";

  algorithm
  assert(sat.Tsat >= T_trip,
    "vapourQuality error: Temperature is lower than triple-point temperature");
  assert(sat.Tsat <= T_crit,
    "vapourQuality error: Temperature is higher than critical temperature");
...
redeclare function extends pressure_dT
  ...
  annotation (derivative=pressure_dT_der, inverse(d=density_pT(
            p=p,
            T=T,
            phase=phase), T=temperature_pd(
            p=p,
            d=d,
            phase=phase)));
  end pressure_dT;
...
redeclare function extends density_pT
  ...
  annotation (inverse(p=pressure_dT(
            d=d,
            T=T,
            phase=phase), T=temperature_pd(
            p=p,
...
redeclare function temperature_ps "returns temperature for given p and d"
  ...
  algorithm
  T := temperature(setState_ps(
      p=p,
      s=s,
      phase=phase));

  annotation (inverse(p=pressure_Ts(
            T=T,
            s=s,
            phase=phase), s=specificEntropy_pT(
            p=p,
            T=T,
            phase=phase)));
  end temperature_ps;
...
redeclare function specificEnthalpy_pT
  ...
  annotation (inverse(T=temperature_ph(
            p=p,
            h=h,
            phase=phase)));
...
redeclare function temperature_ph "returns temperature for given p and h"
  ...
  algorithm
  T := temperature(setState_ph(
      p=p,
      h=h,
      phase=phase));

  annotation (inverse(h=specificEnthalpy_pT(
            p=p,
            T=T,
            phase=phase)));
  end temperature_ph;
...
redeclare function specificEnthalpy_ps
  ...
  annotation (inverse(s=specificEntropy_ph(
            p=p,
            h=h,
            phase=phase)));
...
redeclare function density_ph "returns density for given p and h"
  ...
  algorithm
  d := density(setState_ph(
      p=p,
      h=h,
      phase=phase));

  annotation (derivative=density_ph_der, inverse(h=specificEnthalpy_pd(
            p=p,
            d=d,
            phase=phase)));
  end density_ph;
...
protected
  ...
  elseif (state.phase == 2) then
    ...
    dhp_liq := EoS.dhdT(fl)/EoS.dpdT(fl) + (EoS.dhTd(fl) - EoS.dhdT(fl)*
      EoS.dpTd(fl)/EoS.dpdT(fl))*dTp;
    dhp_vap := EoS.dhdT(fv)/EoS.dpdT(fv) + (EoS.dhTd(fv) - EoS.dhdT(fv)*
      EoS.dpTd(fv)/EoS.dpdT(fv))*dTp;
    ...
    dvph := dvp_liq + dxph*(1.0/sat.vap.d - 1.0/sat.liq.d) + x*(dvp_vap -
      dvp_liq);
...
protected
  ...
  DerEnthalpyByTemperature dhTp=EoS.dhTd(f) - EoS.dhdT(f)*EoS.dpTd(f)/EoS.dpdT(
      f);
  DerTemperatureByPressure dTp=(1.0/sat.vap.d - 1.0/sat.liq.d)/(sat.vap.s - sat.liq.s);
