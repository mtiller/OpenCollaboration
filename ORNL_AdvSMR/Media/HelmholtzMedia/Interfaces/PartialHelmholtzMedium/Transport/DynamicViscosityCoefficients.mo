...
record DynamicViscosityCoefficients
  ...
  constant Real[:, 2] a=fill(
      0.0,
      0,
      2) "coefficients for collision integral";

  // zero density dependence, via Chapman-Enskog-Theory
  constant Real[:, 2] CET=fill(
      0.0,
      0,
      2);
  ...
  constant Real[:, 2] b=fill(
      0.0,
      0,
      2) "coefficients for second viscosity virial coefficent B";

  // residual / high density viscosity contribution
  constant Real[:, 1] c=fill(
      0.0,
      0,
      1) "coefficients for residual viscosity contribution in VS2 model";
  ...
  constant Real[:, 2] g=fill(
      0.0,
      0,
      2) "close-packed density delta_0";
  constant Real[:, 5] e=fill(
      0.0,
      0,
      5) "simple poly";
  constant Real[:, 5] nu_po=fill(
      0.0,
      0,
      5) "  numerator of rational poly";
  constant Real[:, 5] de_po=fill(
      0.0,
      0,
      5) "denominator of rational poly";
