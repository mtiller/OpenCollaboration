...
protected
  ...
  Real[size(dynamicViscosityCoefficients.c, 1), 1] c=
      dynamicViscosityCoefficients.c;
...
algorithm
  ...
  if ((dynamicViscosityModel == DynamicViscosityModel.VS1) or (
    ...
    elseif (dynamicViscosityModel == DynamicViscosityModel.VS1_alternative)
         then
  ...
  elseif (dynamicViscosityModel == DynamicViscosityModel.VS2) then
    ...
    F := G + (c[3, 1] + c[4, 1]*state.T^(-3/2))*dm^0.1 + (c[5, 1] + c[6, 1]/
      state.T + c[7, 1]/state.T^2)*H;
    eta_r := exp(F) - exp(G);
