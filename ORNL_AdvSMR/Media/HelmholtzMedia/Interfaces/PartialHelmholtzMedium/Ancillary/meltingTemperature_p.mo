...
algorithm
  ...
  if (p < p_max) and (p > p_trip) then
    ...
    if (R1*R2 < 0) then
      while (abs(R4/p) > tolerance) and (iter < iter_max) and (abs(T1 - T2) >
          tolerance) loop
        iter := iter + 1;
        T3 := (T1 + T2)/2;
        ...
        if (R4*R3 <= 0) then
          ...
          elseif (R4*R2 < 0) then
            ...
            assert(false,
              "Ancillary.meltingTemperature_p: this should not happen");
          end if;
        end if;
      ...
      assert(iter < iter_max,
        "meltingTemperature_p did not converge, input was p=" + String(p));
      ...
      elseif (abs(R2/p) < tolerance) then
        ...
        assert(false, "Ancillary.meltingTemperature_p: T1=" + String(T1) +
          " and T2=" + String(T2) + " did not bracket the root");
      end if;
    end if;
