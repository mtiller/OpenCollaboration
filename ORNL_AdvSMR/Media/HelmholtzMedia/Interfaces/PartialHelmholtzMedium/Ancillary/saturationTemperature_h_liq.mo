...
algorithm
  ...
  if (h < 0.98*h_crit) and (h > hl_trip) then
    ...
    R2 := h_crit - h;
    if (R1*R2 < 0) then
      while (abs(R4/h) > tolerance) and (iter < iter_max) and (abs(T1 - T2) >
          tolerance) loop
        iter := iter + 1;
        T3 := (T1 + T2)/2;
        ...
        if (R4*R3 <= 0) then
          ...
          elseif (R4*R2 < 0) then
            ...
            assert(false,
              "Ancillary.saturationTemperature_h (liquid side): this should not happen");
          end if;
        end if;
      ...
      assert(iter < iter_max,
        "saturationTemperature_h_liq did not converge, input was h_liq=" +
        String(h));
      ...
      elseif (abs(R2/h) < tolerance) then
        ...
        assert(false, "Ancillary.saturationTemperature_h (liquid side): T1=" +
  ...
  elseif (h <= hl_trip) then
    ...
    assert(false,
      "Ancillary.saturationTemperature_h (liquid side): this should also not happen");
  end if;
