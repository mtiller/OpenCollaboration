within ORNL_AdvSMR.Media.HelmholtzMedia.Interfaces.PartialHelmholtzMedium.Ancillary;
...
algorithm
  ...
  if (d < d_crit) and (d > dv_trip) then
    ...
    R2 := d_crit - d;
    if (R1*R2 < 0) then
      while (abs(R4/d) > tolerance) and (iter < iter_max) and (abs(T1 - T2) >
          tolerance) loop
        iter := iter + 1;
        T3 := (T1 + T2)/2;
        ...
        if (R4*R3 <= 0) then
          ...
          elseif (R4*R2 < 0) then
            ...
            assert(false,
              "Ancillary.saturationTemperature_d (vapour side): this should not happen");
          end if;
        end if;
      ...
      assert(iter < iter_max,
        "saturationTemperature_d_vap did not converge, input was d_vap=" +
        String(d));
      ...
      elseif (abs(R2/d) < tolerance) then
        ...
        assert(false, "Ancillary.saturationTemperature_d (vapour side): T1=" +
          String(T1) + " and T2=" + String(T2) + " did not bracket the root");
      end if;
    end if;
  ...
  elseif (d > d_crit) and (d < dl_trip) then
    ...
    if (R1*R2 < 0) then
      while (abs(R4/d) > tolerance) and (iter < iter_max) and (abs(T1 - T2) >
        ...
        if (R4*R3 <= 0) then
          ...
          elseif (R4*R2 < 0) then
            ...
            assert(false,
              "Ancillary.saturationTemperature_d (liquid side): this should not happen");
          end if;
        end if;
        // Modelica.Utilities.Streams.print("Ridders' method: new brackets T1=" + String(T1) + " and T2=" + String(T2), "printlog.txt");
      end while;
      assert(iter < iter_max,
        "saturationTemperature_d_liq did not converge, remaining residuum is R4="
         + String(R4) + ", input was d_liq=" + String(d));
      ...
      elseif (abs(R2/d) < tolerance) then
        ...
        assert(false, "Ancillary.saturationTemperature_d (liquid side): T1=" +
          String(T1) + " and T2=" + String(T2) + " did not bracket the root");
      end if;
    end if;
