...
protected
  ...
  constant EoS.HelmholtzDerivs fl_trip=EoS.setHelmholtzDerivsFirst(d=dl_trip, T
      =T_trip);
...
algorithm
  ...
  if (s < 0.98*s_crit) and (s > sl_trip) then
    ...
    R2 := s_crit - s;
    if (R1*R2 < 0) then
      while (abs(R4/s) > tolerance) and (iter < iter_max) and (abs(T1 - T2) >
          tolerance) loop
        iter := iter + 1;
        T3 := (T1 + T2)/2;
        ...
        if (R4*R3 <= 0) then
          ...
          elseif (R4*R2 < 0) then
            ...
            assert(false,
              "Ancillary.saturationTemperature_s (liquid side): this should not happen");
          end if;
        end if;
      ...
      assert(iter < iter_max,
        "saturationTemperature_s_liq did not converge, input was s_liq=" +
        String(s));
      ...
      elseif (abs(R2/s) < tolerance) then
        ...
        assert(false, "Ancillary.saturationTemperature_s (liquid side): T1=" +
  ...
  elseif (s <= sl_trip) then
    ...
    assert(false,
      "Ancillary.saturationTemperature_s (liquid side): this should also not happen");
  end if;
