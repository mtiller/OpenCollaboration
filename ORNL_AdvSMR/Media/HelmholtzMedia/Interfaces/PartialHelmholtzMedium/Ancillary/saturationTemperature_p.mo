...
algorithm
  if (p < p_crit) and (p > p_trip) then
    ...
    RES_p := p_crit*exp(tau*sum(n[i]*T_theta^theta[i] for i in 1:
      nPressureSaturation)) - p;

    while ((abs(RES_p/p) > tolerance) and (iter < iter_max)) loop
      iter := iter + 1;

      // calculate gradient of RES_p (= gradient of Wagner equation)
      dpdT := -p_crit*exp(tau*sum(n[i]*T_theta^theta[i] for i in 1:
        nPressureSaturation))*(1/T*sum(n[i]*theta[i]*T_theta^(theta[i] - 1)
        for i in 1:nPressureSaturation) + tau/T*sum(n[i]*T_theta^theta[i] for i
         in 1:nPressureSaturation));
    ...
    assert(iter < iter_max,
      "Ancillary.saturationTemperature_p did not converge, input was p=" +
      String(p));
  ...
  elseif (p >= p_crit) then
    ...
    assert(false,
      "Ancillary.saturationTemperature_p: this should not happen, check p");
  end if;
