...
protected
  ...
  Density d_max=2*fluidLimits.DMAX;
  // extrapolation to higher densities should return reasonable values
...
algorithm
  ...
  if (state.phase == 2) then
    assert(T >= T_trip,
      "setState_Ts_error: pressure is lower than triple point pressure");
    assert(T <= T_crit,
      "setState_TsX_error: pressure is higher than critical pressure");
    sat := setSat_T(T=T);
    assert(s >= sat.liq.s,
      "setState_TsX_error: entropy is lower than saturated liquid entropy: this is single phase liquid");
    assert(s <= sat.vap.s,
      "setState_TsX_error: entropy is higher than saturated vapor entropy: this is single phase vapor");
  else
  ...
  if (state.phase == 2) then
    ...
    assert(iter < iter_max, "setState_Ts did not converge, input was T=" +
      String(T) + " and s=" + String(s));
