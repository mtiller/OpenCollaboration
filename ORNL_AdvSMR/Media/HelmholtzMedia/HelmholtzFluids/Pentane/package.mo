...
extends Interfaces.PartialHelmholtzMedium(
  ...
  Density(
    min=fluidLimitsPentane.DMIN,
    max=fluidLimitsPentane.DMAX,
    start=fluidConstantsPentane.molarMass/fluidConstantsPentane.criticalMolarVolume),

  Temperature(
    min=fluidLimitsPentane.TMIN,
    max=fluidLimitsPentane.TMAX,
    start=298.15),
  AbsolutePressure(
    ...
    start=101325),
  SpecificEnthalpy(
    min=fluidLimitsPentane.HMIN,
    max=fluidLimitsPentane.HMAX,
    start=(fluidLimitsPentane.HMIN + fluidLimitsPentane.HMAX)/2),
  SpecificEntropy(
    min=fluidLimitsPentane.SMIN,
    max=fluidLimitsPentane.SMAX,
    start=(fluidLimitsPentane.SMIN + fluidLimitsPentane.SMAX)/2));
...
final constant Interfaces.PartialHelmholtzMedium.FluidLimits fluidLimitsPentane(
...
final constant Interfaces.PartialHelmholtzMedium.EoS.HelmholtzCoefficients
  ...
  idealLog=[3, 1],
  idealPower=[7.840639316, 0; -84.68310031, 1],
  idealEinstein=fill(
      0.0,
      0,
      2),
  idealCosh=[21.836, 1.789520971],
  idealSinh=[8.95043, 0.380391739; 33.4032, 3.777411113],
  residualPoly=[1.0968643, 0.25, 1.0, 0; -2.9988888, 1.125, 1.0, 0; 0.99516887,
      1.5, 1.0, 0; -0.16170709, 1.375, 2.0, 0; 0.1133446, 0.25, 3.0, 0;
      0.000267606, 0.875, 7.0, 0],
  residualBwr=[0.40979882, 0.625, 2.0, 1; -0.040876423, 1.75, 5.0, 1; -0.38169482,
      3.625, 1.0, 2; -0.10931957, 3.625, 4.0, 2; -0.032073223, 14.5, 3.0, 3;
...
final constant
  ...
  lambda_0_num_coeffs=[1.35558587, 0.0; -0.15569137, -1.0; 1., -96.0],
...
final constant
  ...
  c=[-13.47938293; 1176.62751650; 14.2278439927; -21951.0293411; 0.03766867689;
      70.1529173825; 21435.7720323; 3.215])
  "Coefficients for the dynamic viscosity";

final constant
  Interfaces.PartialHelmholtzMedium.Transport.SurfaceTensionCoefficients
  surfaceTensionCoefficientsPentane(coeffs=[0.0562267, 1.25; -0.0037496, 2.25;
...
final constant
  ...
  pressureSaturation=[-0.73918E+01, 1.0; 0.31102E+01, 1.5; -0.22415E+01, 1.74;
      -0.31585E+01, 3.75; -0.90451E+00, 8.0],
  densityLiquidModel=Interfaces.PartialHelmholtzMedium.Types.DensityLiquidModel.DL1,

  densityLiquid=[0.10178E+01, 0.27; 0.42703E+00, 0.44; 0.11334E+01, 0.6;
      0.41518E+00, 4.0; -0.47950E-01, 5.0],
  densityVaporModel=Interfaces.PartialHelmholtzMedium.Types.DensityVaporModel.DV3,

  densityVapor=[-0.29389E+01, 0.4; -0.62784E+01, 1.18; -0.19941E+02, 3.2; -0.16709E+02,
      6.6; -0.36543E+02, 7.0; -0.12799E+03, 15.0],
  pressureMeltingModel=Interfaces.PartialHelmholtzMedium.Types.PressureMeltingModel.ML1,

  T_reducing=143.47,
  p_reducing=0.076322,
  pressureMelting1=[-8647500000.0, 0; 8647500001.0, 1.649],
