...
redeclare replaceable model extends BaseProperties(
  ...
  import ORNL_AdvSMR.Media.Fluids.IdealGases.Common.SingleGasNasa;
...
protected
  Real[nX] Y(unit="mol/mol") = massToMoleFractions(state.X, data.MM)
    "Molar fractions";
  algorithm
  s := s_TX(state.T, state.X) - sum(state.X[i]*Modelica.Constants.R/MMX[i]*(if
    state.X[i] < Modelica.Constants.eps then Y[i] else Modelica.Math.log(Y[i]*
    state.p/reference_p)) for i in 1:nX);
  end specificEntropy;
...
redeclare function extends specificHelmholtzEnergy
  ...
  extends Modelica.Icons.Function;
  algorithm
  f := h_TX(state.T, state.X) - gasConstant(state)*state.T - state.T*
...
redeclare function extends specificHeatCapacityCv
  ...
  algorithm
  cv := {SingleGasNasa.cp_T(data[i], state.T) for i in 1:nX}*state.X - data.R*
    state.X;
...
redeclare function extends isentropicEnthalpy "Return isentropic enthalpy"
  ...
  h_is := if exact then specificEnthalpy_psX(
      ...
      refState.X) else isentropicEnthalpyApproximation(p_downstream, refState);
...
redeclare function density_derX "Return density derivative by mass fraction"
  ...
  algorithm
  dddX := {-state.p/(state.T*gasConstant(state))*molarMass(state)/data[i].MM
    for i in 1:nX};
  end density_derX;
