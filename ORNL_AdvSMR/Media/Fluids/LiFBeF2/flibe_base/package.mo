...
partial package flibe_base "Flibe properties"


extends Interfaces.PartialSinglePhaseMedium(
  mediumName="LiFBeF2",
  substanceNames={"LiFBeF2"},
  singleState=true,
  ...
  smoothModel=true,
  onePhase=true,
  fluidConstants=ORNL_AdvSMR.Media.Fluids.LiFBeF2.flibeConstants);
...
protected
  ...
  algorithm
  // d := (2413.11 - 0.4884*h/2380)*(1 + 5.9361e-11*p);
  // d := (2413.11 - 0.4884*h/2380)*(1 + 2.3e-11*exp(0.001*h/2380)*p);  - (pressure dependence creates nonlinearity; eliminated...   Apr. 23, 2013 by Sacit Cetiner
  d := 2413.11 - 0.4884*h/2380 annotation (smoothOrder=20);
  end density_ph;
...
redeclare function density_pT
  ...
  output Density d "Density";
  algorithm
  // d := (2413.11 - 0.4884*T)*(1 + 5.9361e-11*p) "T in [800-1080 K]";
  // d := (2413.11 - 0.4884*T)*(1 + 2.3e-11*exp(0.001*T)*p) "T in [800-1080 K]"; - pressure term eliminated, Apr. 23, 2013 by Sacit Cetiner
  d := 2413.11 - 0.4884*T "T in [800-1080 K]";
  // d := 2413.11;
  end density_pT;
...
redeclare function pressure_dT
  ...
  // p := (d/(3658.28 - 0.887*T) - 1)/5.9361e-11 "inverse of the d(p,T) equation";
  p := 1.46e11*exp(-24180/T);
  end pressure_dT;
...
redeclare function extends dynamicViscosity "Dynamic viscosity of flibe"
  ...
  eta := 1.16e-4*exp(3755/state.T)
    "dynamic viscosity (Pa ) as a function of temperature (K)";
  // eta := 1 * (0.0721 - 6.5279e-5*state.T) "Linearized dynamic viscosity (Pa s)";
  // eta := 0.2546 - 4.8640e-4*state.T + 2.3396e-7*state.T^2;
...
redeclare function extends isothermalCompressibility
  ...
  algorithm
  if ph_explicit then
    kappa := 2.3e-11*Math.exp(0.001*state.T);
  elseif pT_explicit then
    kappa := 2.3e-11*Math.exp(0.001*state.T);
  else
    kappa := 2.3e-11*Math.exp(0.001*state.T);
  end if;
  // N. A. Anderson, Nuclear Technology v. 178, p. 335. Cetiner: Updated Feb. 6, 2013
...
redeclare function extends velocityOfSound
  ...
  // velocity of sound in water at 20 deg C is used (Sacit: Feb. 9)
  // velocity of sound calculated as a = sqrt(del_p/del_rho) (Sacit: Apr. 22)
  end velocityOfSound;
...
protected
  final AbsolutePressure p0=2.145e-5 "Triple-point pressure (Pa)";
  algorithm
  /*   ddhp := f(state.p, state.h, state.phase) */
  // ddhp := -0.4884/2380*(1 + 5.9361e-11*state.p);     - obsolete (Apr. 20, 2013) by Sacit Cetiner
  /* ddhp := -0.4884/2380 * (1 + 2.3e-11*exp(0.001*state.h/2380)*state.p)
          + 2.3e-14/2380 * (2413.11 - 0.4884*state.h/2380) * exp(0.001*state.h/2380)*state.p;  - pressure eliminated, Apr. 23, 2013 by Sacit Cetiner */
  ddhp := -0.4884/2380;
  // ddhp := 0;
  end density_derh_p;
...
redeclare function extends density_derp_h "density derivative by pressure"
  algorithm
  /*   ddph := f(state.p, state.h, state.phase) */
  // ddph := 5.9361e-11*(2413.3 - 0.4884*state.h/2380);    - obsolete (Apr. 20, 2013 by Sacit Cetiner)
  // ddph := 2.3e-11 * exp(0.001*state.h/2380) * (2413.11 - 0.4884*state.h/2380); - pressure term eliminated, Apr. 23, 2013 by Sacit M. Cetiner
  ddph := 0;
  end density_derp_h;
...
redeclare function extends saturationPressure "saturation pressure of flibe"
  algorithm
  p := 101325;
