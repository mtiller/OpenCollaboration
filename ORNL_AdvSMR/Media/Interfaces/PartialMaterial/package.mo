...
partial package PartialMaterial "Partial medium properties (base package of all media packages)"
...
constant
  ORNL_AdvSMR.Media.Interfaces.PartialMaterial.Choices.IndependentVariables
...
final constant Integer nC=size(extraPropertiesNames, 1)
  ...
  annotation (Evaluate=true);
constant Real C_nominal[nC](min=fill(Modelica.Constants.eps, nC)) = 1.0e-6*ones(
  nC) "Default for the nominal values for the extra properties";
...
replaceable partial model BaseProperties
  ...
  if standardOrderComponents then
    ...
    for i in 1:nX loop
      assert(X[i] >= -1.e-5 and X[i] <= 1 + 1.e-5, "Mass fraction X[" + String(
        i) + "] = " + String(X[i]) + "of substance " + substanceNames[i] +
