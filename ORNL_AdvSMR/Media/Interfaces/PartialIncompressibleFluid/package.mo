...
partial package PartialIncompressibleFluid "Generic pure incompressible liquid model with constant cp"

import ORNL_AdvSMR.Media.*;
import SI = Modelica.SIunits;


extends Interfaces.PartialPureSubstance(ThermoStates=Choices.IndependentVariables.pTX,
    singleState=false);
...
redeclare function extends setState_phX
  ...
  algorithm
  state := ThermodynamicState(p=p, T=(h - reference_h - (p - reference_p)*((1
     - beta_const*reference_T)/reference_d))/cp_const + reference_T);
  end setState_phX;
...
redeclare function extends setSmoothState
  ...
  algorithm
  state := ThermodynamicState(p=Common.smoothStep(
      x,
      state_a.p,
      state_b.p,
      x_small), T=Common.smoothStep(
      x,
      state_a.T,
      state_b.T,
      x_small));
...
redeclare function extends specificEnthalpy
  ...
  algorithm
  h := reference_h + (state.T - reference_T)*cp_const + (state.p - reference_p)
    *(1 - beta_const*reference_T)/reference_d;
...
redeclare function extends velocityOfSound
  ...
  extends Modelica.Icons.Function;
  algorithm
  a := sqrt(max(0, 1/(kappa_const*density(state) - beta_const*beta_const*state.T
    /cp_const)));
  end velocityOfSound;
...
*/
  ...
  */
  ...
  h_is := specificEnthalpy(setState_pTX(
      p_downstream,
      reference_T/(1 - ((refState.T - reference_T)/refState.T + (refState.p -
      p_downstream)*(-beta_const/(reference_d*cp_const)))),
      reference_X));
...
redeclare function extends specificHeatCapacityCv
  ...
  algorithm
  cv := if constantJacobian then cp_const - reference_T*beta_const*beta_const/(
    kappa_const*reference_d) else state.T*beta_const*beta_const/(kappa_const*
    reference_d);
