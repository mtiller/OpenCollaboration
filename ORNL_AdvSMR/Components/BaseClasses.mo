...
package BaseClasses
  ...
  partial model PartialLumpedVessel
    ...
    extends ORNL_AdvSMR.Interfaces.PartialLumpedVolume;
    ...
    VesselFluidPorts_b ports[nPorts](redeclare each package Medium = Medium)
      "Fluid inlets and outlets" annotation (Placement(transformation(extent={{
              -40,-10},{40,10}}, origin={0,-100})));
    ...
    parameter ORNL_AdvSMR.Components.BaseClasses.VesselPortsData[if
      use_portsData then nPorts else 0] portsData "Data of inlet/outlet ports"
      annotation (Dialog(
        tab="General",
        group="Ports",
        enable=use_portsData));

    parameter Medium.MassFlowRate m_flow_nominal=if system.use_eps_Re then
        system.m_flow_nominal else 1e2*system.m_flow_small
      "Nominal value for mass flow rates in ports" annotation (Dialog(
        tab="Advanced",
...
*/
    ...
    replaceable model HeatTransfer =
        ORNL_AdvSMR.Components.BaseClasses.HeatTransfer.IdealHeatTransfer
      constrainedby
      ORNL_AdvSMR.Components.BaseClasses.HeatTransfer.PartialVesselHeatTransfer
...
");
    ...
    for i in 1:nPorts loop
      portInDensities[i] = Medium.density(Medium.setState_phX(
          vessel_ps_static[i],
      ...
      if use_portsData then
        ...
        m_flow_turbulent[i] = if not use_Re then m_flow_small else max(
          m_flow_small, (Modelica.Constants.pi/8)*portsData_diameter[i]*(
          Medium.dynamicViscosity(Medium.setState_phX(
            vessel_ps_static[i],
            inStream(ports[i].h_outflow),
            inStream(ports[i].Xi_outflow))) + Medium.dynamicViscosity(medium.state))
          *Re_turbulent);
      else
      ...
      inFlow[i] = not regularFlow[i] and (s[i] > 0 or portsData_height[i] >=
        fluidLevel_max);
...
Ideal heat transfer without thermal resistance.
    ...
    equation
      Q_flows = {(alpha0 + k)*surfaceAreas[i]*(heatPorts[i].T - Ts[i]) for i
         in 1:n};
...
Heat transfer correlations for pipe models
  ...
  record VesselPortsData "Data to describe inlet/outlet ports at vessels:
    ...
    parameter Modelica.SIunits.Height height=0
      "Height over the bottom of the vessel";
    ...
    annotation (
      ...
      Diagram(coordinateSystem(
          ...
          initialScale=0.2), graphics={Text(extent={{-75,130},{75,100}},
            textString="%name"),Rectangle(extent={{-25,100},{25,-100}},
