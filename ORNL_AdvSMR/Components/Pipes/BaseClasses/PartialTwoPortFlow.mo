...
partial model PartialTwoPortFlow "Base class for distributed flow models"
  ...
  extends Modelica.Fluid.Interfaces.PartialDistributedVolume(final n=nNodes,
  ...
  parameter Boolean useLumpedPressure=false
    ...
    annotation (Dialog(tab="Advanced"), Evaluate=true);
  final parameter Integer nFM=if useLumpedPressure then nFMLumped else
      nFMDistributed "number of flow models in flowModel";
  final parameter Integer nFMDistributed=if modelStructure == ModelStructure.a_v_b
       then n + 1 else if (modelStructure == ModelStructure.a_vb or
      modelStructure == ModelStructure.av_b) then n else n - 1;
  final parameter Integer nFMLumped=if modelStructure == ModelStructure.a_v_b
       then 2 else 1;
  ...
  FlowModel flowModel(
    ...
    final g=system.g) "Flow model" annotation (Placement(transformation(extent=
            {{-77,-38},{75,-20}}, rotation=0)));

  // Flow quantities
  Medium.MassFlowRate[n + 1] m_flows(each min=if allowFlowReversal then -
        Modelica.Constants.inf else 0, each start=m_flow_start)
...
equation
  ...
  if useLumpedPressure then
    if modelStructure <> ModelStructure.a_v_b then
      ...
      if n == 1 then
        ...
        // n > 1
        crossAreasFM[1:2] = {sum(crossAreas[1:iLumped - 1])/(iLumped - 1),sum(
          crossAreas[iLumped:n])/(n - iLumped + 1)};
        dimensionsFM[1:2] = {sum(dimensions[1:iLumped - 1])/(iLumped - 1),sum(
          dimensions[iLumped:n])/(n - iLumped + 1)};
        roughnessesFM[1:2] = {sum(roughnesses[1:iLumped - 1])/(iLumped - 1),sum(
          roughnesses[iLumped:n])/(n - iLumped + 1)};
      end if;
    else
      if n == 1 then
        ...
        crossAreasFM[1:3] = {sum(crossAreas[1:iLumped - 1])/(iLumped - 1),sum(
          crossAreas)/n,sum(crossAreas[iLumped:n])/(n - iLumped + 1)};
        dimensionsFM[1:3] = {sum(dimensions[1:iLumped - 1])/(iLumped - 1),sum(
          dimensions)/n,sum(dimensions[iLumped:n])/(n - iLumped + 1)};
        roughnessesFM[1:3] = {sum(roughnesses[1:iLumped - 1])/(iLumped - 1),sum(
          roughnesses)/n,sum(roughnesses[iLumped:n])/(n - iLumped + 1)};
      end if;
    end if;
  else
    if modelStructure == ModelStructure.av_vb then
      ...
      if n == 2 then
        ...
        pathLengths[1:n - 1] = cat(
          1,
          {lengths[1] + 0.5*lengths[2]},
          0.5*(lengths[2:n - 2] + lengths[3:n - 1]),
          {0.5*lengths[n - 1] + lengths[n]});
        dheightsFM[1:n - 1] = cat(
          1,
          {dheights[1] + 0.5*dheights[2]},
          0.5*(dheights[2:n - 2] + dheights[3:n - 1]),
          {0.5*dheights[n - 1] + dheights[n]});
      end if;
    ...
    elseif modelStructure == ModelStructure.av_b then
      ...
      crossAreasFM[1:n + 1] = cat(
        1,
        crossAreas[1:n],
        {crossAreas[n]});
      dimensionsFM[1:n + 1] = cat(
        1,
        dimensions[1:n],
        {dimensions[n]});
      roughnessesFM[1:n + 1] = cat(
        1,
        roughnesses[1:n],
        {roughnesses[n]});
    ...
    elseif modelStructure == ModelStructure.a_v_b then
      ...
      pathLengths[1:n + 1] = cat(
        1,
        {0.5*lengths[1]},
        0.5*(lengths[1:n - 1] + lengths[2:n]),
        {0.5*lengths[n]});
      dheightsFM[1:n + 1] = cat(
        1,
        {0.5*dheights[1]},
        0.5*(dheights[1:n - 1] + dheights[2:n]),
        {0.5*dheights[n]});
      crossAreasFM[1:n + 2] = cat(
        1,
        {crossAreas[1]},
        crossAreas[1:n],
        {crossAreas[n]});
      dimensionsFM[1:n + 2] = cat(
        1,
        {dimensions[1]},
        dimensions[1:n],
        {dimensions[n]});
      roughnessesFM[1:n + 2] = cat(
        1,
        {roughnesses[1]},
        roughnesses[1:n],
        {roughnesses[n]});
  ...
  if useLumpedPressure then
    ...
    if modelStructure <> ModelStructure.a_v_b then
      vsFM[1] = vs[1:iLumped - 1]*lengths[1:iLumped - 1]/sum(lengths[1:iLumped
         - 1]);
...
<li><i>04 Mar 2006</i>
    ...
    Diagram(coordinateSystem(
        ...
        grid={1,1}), graphics={Polygon(
          ...
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="flowModel.dps_fg[1]"),Line(
          points={{0,-70},{100,-70}},
          arrow={Arrow.None,Arrow.Filled},
          color={0,0,0}),Text(
          ...
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="flowModel.dps_fg[2:n-1]"),Line(
          points={{-95,0},{-5,0}},
          arrow={Arrow.None,Arrow.Filled},
          color={0,0,0}),Text(
          extent={{-49,7},{-19,1}},
          ...
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="(ModelStructure av_vb, n=3)"),Line(
          ...
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="states[2:n-1]"),Ellipse(
          extent={{95,5},{105,-5}},
          pattern=LinePattern.None,
          lineColor={0,0,0},
          fillColor={0,0,0},
          ...
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="states[1]"),Text(
          extent={{-99.5,30},{-69.5,24}},
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="dimensions[1]"),Text(
          extent={{-0.5,30},{40,24}},
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="dimensions[2:n-1]"),Text(
          extent={{100.5,30},{130.5,24}},
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="dimensions[n]"),Line(
          points={{-50,73},{-50,52}},
          smooth=Smooth.None,
          color={0,0,0},
          pattern=LinePattern.Dot),Line(
          ...
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="vs[2:n-1]"),Text(
          extent={{-80,18},{-70,12}},
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="vs[1]"),Line(
          points={{-100,11},{-50,11}},
          arrow={Arrow.None,Arrow.Filled},
          color={0,0,0}),Text(
          extent={{70,18},{80,12}},
          ...
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="flowModel.pathLengths[1]"),Line(
          points={{-100,-82},{0,-82}},
          arrow={Arrow.Filled,Arrow.Filled},
          color={0,0,0}),Line(
          points={{0,-82},{100,-82}},
          ...
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="flowModel.pathLengths[2:n-1]"),Text(
          extent={{-100,77},{-50,71}},
          lineColor={0,0,255},
          pattern=LinePattern.None,
          textString="lengths[1]"),Text(
          extent={{50,77},{100,71}},
          lineColor={0,0,255},
          pattern=LinePattern.None,
