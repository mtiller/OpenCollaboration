...
model UpperPlenum_3i "Joins two water/steam flows"
...
equation
  ...
  out.h_outflow = homotopy(if (in2.m_flow < 0 and rev_in2) then inStream(in1.h_outflow)
    ...
    inStream(in1.h_outflow)*(in1.m_flow + wzero) + inStream(in1.h_outflow)*(in2.m_flow
     + wzero))/(in1.m_flow + 2*wzero + in2.m_flow), (inStream(in1.h_outflow)*(
    in1.m_flow + wzero) + inStream(in1.h_outflow)*(in2.m_flow + wzero))/(in1.m_flow
     + 2*wzero + in2.m_flow));
