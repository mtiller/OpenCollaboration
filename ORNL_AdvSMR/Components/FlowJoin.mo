...
equation
  ...
  out.h_outflow = homotopy(if (in2.m_flow < 0 and rev_in2) then inStream(in1.h_outflow)
     else if (in1.m_flow < 0 and rev_in1) then inStream(in2.h_outflow) else (
    inStream(in1.h_outflow)*(in1.m_flow + wzero) + inStream(in2.h_outflow)*(in2.m_flow
     + wzero))/(in1.m_flow + 2*wzero + in2.m_flow), (inStream(in1.h_outflow)*(
    in1.m_flow + wzero) + inStream(in2.h_outflow)*(in2.m_flow + wzero))/(in1.m_flow
     + 2*wzero + in2.m_flow));
  in1.h_outflow = homotopy(if (in2.m_flow < 0 and rev_in2) then inStream(out.h_outflow)
     else if (out.m_flow < 0 or not rev_out) then inStream(in1.h_outflow) else
    (inStream(out.h_outflow)*(out.m_flow + wzero) + inStream(in1.h_outflow)*(
    in1.m_flow + wzero))/(out.m_flow + 2*wzero + in2.m_flow), inStream(out.h_outflow));
