...
model PipeFlow
  ...
  Modelica.SIunits.DynamicViscosity[nNodes] mu "Dynamic viscosity";
  Modelica.SIunits.ReynoldsNumber[nNodes] Re "Reynolds number";
  Modelica.SIunits.PrandtlNumber[nNodes] Pr "Prandtl number";
  Modelica.SIunits.PecletNumber[nNodes] Pe "Peclet number";
  Modelica.SIunits.NusseltNumber[nNodes] Nu "Nusselt number";
  Modelica.SIunits.CoefficientOfHeatTransfer[nNodes] h_conv
    "Coefficient of convective heat transfer (W/m2K)";
  Modelica.SIunits.SpecificHeatCapacity[nNodes] cp "Specific heat capacity";
  Modelica.SIunits.ThermalConductivity k[nNodes] "Fluid thermal conductivity";
...
protected
  ...
  Modelica.SIunits.HeatFlux phibar[nNodes - 1] "Average heat flux";
...
equation
  ...
  for j in 1:nNodes - 1 loop
    if Medium.singleState then
      A*l*rhobar[j]*der(htilde[j]) + wbar[j]*(h[j + 1] - h[j]) = l*omega*phibar[
        j] "Energy balance (pressure effects neglected)";
    else
  ...
  for j in 1:nNodes loop
    ...
    u[j] = w/(rho[j]*A);
    Re[j] = Medium.density(fluidState[j])*abs(u[j])*Dhyd/
      Medium.dynamicViscosity(fluidState[j]);
    Pr[j] = Medium.specificHeatCapacityCp(fluidState[j])*
      Medium.dynamicViscosity(fluidState[j])/Medium.thermalConductivity(
      fluidState[j]);
  end for;
  ...
  Pe = Re .* Pr;
  // Nu = 4.496*(-16.15 + 24.96*PoD - 8.55*PoD^2)*ones(nNodes);
  if Q >= 0 then
    Nu = homotopy(0.023 .* Re .^ 0.8 .* Pr .^ 0.4, 75) "Heating condition";
  else
    Nu = homotopy(0.023 .* Re .^ 0.8 .* Pr .^ 0.3, 75) "Cooling condition";
  end if;
  h_conv = Nu .* k/Dhyd/2;
  // h_conv = Nu .* k/Dhyd;
  wall.gamma = h_conv;
  ...
  Qdot = Nt*w*(h[nNodes] - h[1]);
  Qdot_actual = Nt*w*(inStream(outfl.h_outflow) - inStream(infl.h_outflow));
  ...
  T = wall.T;
  phibar = (wall.phi[1:nNodes - 1] + wall.phi[2:nNodes])/2;
