...
model Flow1D2ph
  ...
  extends Flow1DBase(redeclare replaceable package Medium =
        ORNL_AdvSMR.Media.Fluids.Water.StandardWater constrainedby
      Modelica.Media.Interfaces.PartialTwoPhaseMedium "Medium model",
      FluidPhaseStart=ORNL_AdvSMR.Choices.FluidPhase.FluidPhases.TwoPhases);
...
equation
  ...
  for j in 1:(N - 1) loop
    ...
    dMdt[j] = A*l*(drbdh1[j]*der(h[j]) + drbdh2[j]*der(h[j + 1]) + drbdp[j]*der(
      p)) "Mass balance for each volume";
    ...
    dpf[j] = (if FFtype == FFtypes.NoFriction then 0 else homotopy(smooth(1, Kf[
      j]*squareReg(w, wnom/Nt*wnf))*vbar[j], dpnom/(N - 1)/(wnom/Nt)*w));
    ...
    elseif noEvent(h[j] < hl and h[j + 1] >= hl and h[j + 1] <= hv) then
      ...
      drbdp[j] = ((drdp[j] + drldp)*(hl - h[j])/2 + (rho[j] + rhol)/2*dhldp +
        ...
        *drdp[j + 1]))/(h[j + 1] - h[j]);
      drbdh1[j] = (rhobar[j] - (rho[j] + rhol)/2 + drdh[j]*(hl - h[j])/2)/(h[j
         + 1] - h[j]);
      drbdh2[j] = (rho[j + 1] - rhobar[j])/(h[j + 1] - h[j]);
    ...
    elseif noEvent(h[j] > hv and h[j + 1] < hl) then
      ...
      drbdp[j] = ((drdp[j] + drvdp)*(hv - h[j])/2 + (rho[j] + rhov)/2*dhvdp +
        ...
        h[j + 1] - h[j]);
      drbdh1[j] = (rhobar[j] - (rho[j] + rhov)/2 + drdh[j]*(hv - h[j])/2)/(h[j
         + 1] - h[j]);
      drbdh2[j] = ((rhol + rho[j + 1])/2 - rhobar[j] + drdh[j + 1]*(h[j + 1] -
        hl)/2)/(h[j + 1] - h[j]);
    else
