...
model SteamTurbineUnit "Turbine for steam flows"
  ...
  extends ORNL_AdvSMR.Icons.Water.SteamTurbineUnit;
  ...
  parameter ORNL_AdvSMR.Choices.Init.Options initOpt=ORNL_AdvSMR.Choices.Init.Options.steadyState
  ...
  ORNL_AdvSMR.Interfaces.FlangeA inlet(redeclare package Medium = Medium,
    ...
    annotation (Placement(transformation(extent={{-120,50},{-80,90}}, rotation=
            0)));
  ORNL_AdvSMR.Interfaces.FlangeB outlet(redeclare package Medium = Medium,
      m_flow(max=if allowFlowReversal then +Modelica.Constants.inf else 0))
    annotation (Placement(transformation(extent={{80,-90},{120,-50}}, rotation=
            0)));
