...
model BundleFlow "1-dimensional fluid flow model through bundles"
  ...
  HeatTransfer heatTransfer(
    ...
    final v=5,
    final use_k=use_HeatTransfer) "Heat transfer model" annotation (Dialog(
        enable=false), Placement(transformation(extent={{-40,40},{40,60}},
          rotation=0)));
...
initial equation
  ...
  if initOpt == ORNL_AdvSMR.Choices.Init.Options.noInit then
    ...
    htilde = linspace(
      ...
      hstartout,
      nNodes - 1);
    // Modelica.Utilities.Streams.print("  - NO INITIALIZATION - ", "");
