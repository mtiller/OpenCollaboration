...
model CondPlant_cc
  ...
  ThermoPower3.PowerPlants.Control.PID pID(
    ...
    Ti=2000,
    Kp=-3,
    steadyStateInit=SSInit) annotation (Placement(transformation(extent={{-40,
            34},{-60,54}}, rotation=0)));
  ...
  Condensers.CondenserPreP_tap condenserIdeal_tap(
    ...
    p=p,
    Vtot=Vtot,
    Vlstart=Vlstart) annotation (Placement(transformation(extent={{16,-26},{76,
            34}}, rotation=0)));
  ThermoPower3.Water.SourceW sourceTap(
    redeclare package Medium = FluidMedium,
    p0=p,
    allowFlowReversal=true) annotation (Placement(transformation(extent={{-80,-30},
            {-60,-10}}, rotation=0)));
  ThermoPower3.PowerPlants.SteamTurbineGroup.Components.Comp_bubble_h
    ...
    annotation (Placement(transformation(extent={{-30,-10},{-60,10}}, rotation=
            0)));
...
equation
  ...
  connect(pID.CS, sourceTap.in_w0)
    annotation (Line(points={{-60,44},{-74,44},{-74,-14}}, color={0,0,127}));
  connect(pID.PV, condenserIdeal_tap.ratio_Vv_Vtot) annotation (Line(points={{-40,
          40},{-28,40},{-28,-2},{16,-2}}, color={0,0,127}));
  ...
  connect(condenserIdeal_tap.waterOut, WaterOut) annotation (Line(
      ...
      thickness=0.5,
      color={0,0,255}));
  connect(BubbleEnthalpy.h, sourceTap.in_h)
    annotation (Line(points={{-58.5,0},{-66,0},{-66,-14}}, color={0,0,127}));
  ...
  connect(SensorsBus.Cond_Q, condenserIdeal_tap.Qcond) annotation (Line(points=
          {{98,-40},{-8,-40},{-8,16},{16,16}}, color={255,170,213}));
