...
public
  Modelica.Blocks.Sources.Ramp com_valveHP_a(
    ...
    offset=0.2,
    duration=5,
    startTime=1000) annotation (Placement(transformation(extent={{0,76},{-12,88}},
          rotation=0)));
public
  Modelica.Blocks.Sources.Ramp com_valveHP_b(
    ...
    offset=0,
    duration=5,
    startTime=2000) annotation (Placement(transformation(extent={{0,54},{-12,66}},
          rotation=0)));
  Modelica.Blocks.Math.Add add annotation (Placement(transformation(
        origin={-49,69},
        extent={{-7,-7},{7,7}},
        rotation=180)));
  ORNL_AdvSMR.PRISM.PowerConversionSystem.ElectricGenerators.OldElementsSwingEquation.Generator_SE_com
    generator_SE(
    J=7000,
    delta_start=0.77,
    Pmax=20e6,
...
equation
  ...
  connect(grid.connection, generator_SE.powerConnection) annotation (Line(
      ...
      thickness=0.5));
  annotation (Diagram(graphics), experiment(StopTime=3000, NumberOfIntervals=
          30000));
