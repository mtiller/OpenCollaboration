within ORNL_AdvSMR.PRISM.PowerConversionSystem.GasTurbineGroup.Tests;
model TestGasTurbine

  Examples.GasTurbineSimplified gasTurbine annotation (Placement(transformation(
          extent={{-40,-40},{0,0}}, rotation=0)));
  ThermoPower3.Gas.SinkP sinkP(redeclare package Medium =
        ThermoPower.Media.FlueGas) annotation (Placement(transformation(extent=
            {{62,-14},{82,6}}, rotation=0)));
  Modelica.Blocks.Sources.Constant const annotation (Placement(transformation(
          extent={{-90,-30},{-70,-10}}, rotation=0)));
  ThermoPower3.PowerPlants.HRSG.Components.StateReader_gas stateReader_gas(
