...
model superCriticalWaterSystem2
  ...
  ThermoPower3.PowerPlants.SteamTurbineGroup.Examples.CondPlant_cc
    ...
    SSInit=SSInit,
    p=5398.2) annotation (Placement(transformation(extent={{-30,-30},{30,30}},
          rotation=0)));
  ThermoPower3.PowerPlants.HRSG.Components.PrescribedSpeedPump totalFeedPump(
    ...
    nominalFlow=89.8,
    n0=1500,
    redeclare package WaterMedium = ORNL_AdvSMR.StandardWater,
  ...
  Modelica.Blocks.Sources.BooleanStep booleanStep(startTime=400, startValue=
        true) annotation (Placement(transformation(extent={{95,60},{80,75}})));
equation
  connect(steamTurbineStodola.inlet, sourceP.flange) annotation (Line(
      ...
      thickness=1,
      smooth=Smooth.None));
  connect(steamTurbineStodola.outlet, controlledCondeser.SteamIn) annotation (
      Line(
