...
algorithm
  tau := data.TCRIT/T;
  delta := if (d == data.DCRIT and T == data.TCRIT) then 1 +
    Modelica.Constants.eps else d/data.DCRIT;
  o[1] := tau*tau;
  o[2] := o[1]*o[1];
  o[3] := o[2]*o[2];
  o[4] := o[3]*tau;
  ...
  fdelta := (1.0658070028513 + delta*(o[1]*(-1.2654315477714 +
    ...
    delta*(-0.118674762819776*o[12] + delta*(o[1]*(
  ...
  ftau := 20.944396974307 + tau*(-15.3735415757432 + o[2]*tau*(
    ...
    4.27343239646986*o[11])*tau + delta*(-0.027006744482696*tau
     + delta*(-0.385692979164272*o[13] + delta*(delta*(-0.00016557679795037
