...
algorithm
  ...
  if supercritical then
    dguess := dmin + (T - data.TLIMIT1)/(data.TLIMIT1 - Tmax)*(
      dmax - dmin);
  ...
  assert(error <> 1,
    "error in inverse function dofpt3: iteration failed");
end dofpt3;
