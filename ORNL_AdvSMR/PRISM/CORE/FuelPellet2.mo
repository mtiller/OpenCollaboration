...
model FuelPellet2
  "1-D model of thermal model for generalized fuel pins with no heat port"
  ...
  Temperature T_co "Cladding outside temperature";
  Temperature T_cool "Coolant temperature";
  HeatFlux q_pp_cond "Heat flux calculated from conduction";
  HeatFlux q_pp_conv "Heat flux calculated from convection";
...
equation
  ...
  T_co = T_c[radNodes_c];
  q_pp_cond = -k_c[radNodes_c]*(T_c[1] - T_c[2])/delta_c;
  q_pp_conv = h*(T_c[radNodes_c] - T_cool);
