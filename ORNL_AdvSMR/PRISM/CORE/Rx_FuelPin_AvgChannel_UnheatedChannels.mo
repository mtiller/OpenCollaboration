...
model Rx_FuelPin_AvgChannel_UnheatedChannels
  ...
  parameter Integer noFlowChannels=integer(noFuelAssemblies*
      noFuelPinsPerAssembly/(2*1.0419))
    "Total number of flow channels in the core"
    annotation (Dialog(tab="General", group="Core Loading Parameters"));
  ...
  Components.PipeFlow channel(
    ...
    pstart=100000,
    Nt=noFlowChannels) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={80,0})));
  ...
  ReactorKinetics reactorKinetics(
    noAxialNodes=noAxialNodes,
    alpha_c=alpha_c,
    noFuelAssemblies=noFuelAssemblies,
    noFuelPins=noFuelPinsPerAssembly,
    alpha_f=alpha_f,
    Q_nom=Q_nom,
    H_fuelPin=H_fuelPin,
    T_f0=T_f0,
  ...
  Modelica.Blocks.Sources.Trapezoid w(
    ...
    amplitude=0,
    offset=w_nom)
    annotation (Placement(transformation(extent={{35,-93},{50,-78}})));

  Components.PipeFlow channel1(
    FFtype=ORNL_AdvSMR.Choices.Flow1D.FFtypes.Colebrook,
    rhonom=900,
    Cfnom=0.005,
    FluidPhaseStart=ORNL_AdvSMR.Choices.FluidPhase.FluidPhases.Liquid,
...
equation
  ...
  reactorKinetics.T_fe = sum(fuelPin.fp.T_f[1])/noAxialNodes;
  convec.fluidside.gamma = 50e3*ones(noAxialNodes);
  ...
  connect(w.y, channelIn.in_w0) annotation (Line(
      points={{50.75,-85.5},{75.5,-85.5}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(channel2.outfl, channelOut.flange) annotation (Line(
      points={{80,60},{80,80}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(channelIn.flange, channel1.infl) annotation (Line(
      points={{80,-75},{80,-60}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(channel1.outfl, channel.infl) annotation (Line(
      points={{80,-40},{80,-10}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(channel.outfl, channel2.infl) annotation (Line(
      points={{80,10},{80,40}},
      color={0,127,255},
      smooth=Smooth.None));
  annotation (
...
end Rx_FuelPin_AvgChannel_UnheatedChannels;
