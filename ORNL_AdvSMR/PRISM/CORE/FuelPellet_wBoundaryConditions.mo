...
model FuelPellet_wBoundaryConditions
  ...
  parameter Real P(
    min=0.0,
    max=1.0) = 0.0 "Fuel porosity fraction" annotation (
    Evaluate=true,
    choicesAllMatching=true,
    Dialog(tab="Material Properties", group="Fuel"));
  parameter Real P_Na(
    min=0.0,
    max=1.0) = 0.0 "Fraction of sodium infiltration into the fuel metal"
    annotation (
    Evaluate=true,
    choicesAllMatching=true,
    Dialog(tab="Material Properties", group="Fuel"));
  parameter Boolean varConductivity=true
    ...
    choicesAllMatching=true,
    Dialog(tab="Material Properties", group="Fuel"));
  Real W_U=1.0 - (W_Pu + W_Zr) "Uranium weight percent";
...
public
  SIInterfaces.SIPowerInput powerIn annotation (Placement(transformation(extent
          ={{-100,-10},{-80,10}}), iconTransformation(extent={{-100,-10},{-80,
            10}})));
  replaceable ORNL_AdvSMR.Thermal.HT wall constrainedby ORNL_AdvSMR.Thermal.HT
    "Heat transfer port to the coolant" annotation (
    Evaluate=true,
    choicesAllMatching=true,
...
equation
  ...
  k_f0 = a .+ b .* T_f .+ c .* T_f .^ 2 "W/mK";
  // k_f = k_f0 "W/mK - Sodium infiltration neglected...";
  k_f = 7.5*ones(radNodes_f) "(W/mK)";
  alpha_f = k_f ./ (rho_f .* cp_f);
  // Finite difference in center node
  1/alpha_f[1]*der(T_f[1]) = 4/delta_f^2*(T_f[2] - T_f[1]) + q_ppp/k_f[1];
  // Finite difference in interior nodes
  for i in 2:radNodes_f - 1 loop
    1/alpha_f[i]*der(T_f[i]) = 1/delta_f^2*(T_f[i - 1] - 2*T_f[i] + T_f[i + 1])
       + 1/(i*delta_f)*1/(2*delta_f)*(T_f[i + 1] - T_f[i - 1]) + q_ppp/k_f[i];
  end for;
  ...
  1/alpha_f[radNodes_f]*der(T_f[radNodes_f]) = 1/delta_f^2*(T_f[radNodes_f - 1]
     ...
     + q_ppp/k_f[radNodes_f];

  /* GAP */
  rho_g = 1017.0 - 0.239*T_g
    "Linearized version of density equation by Stone et al.";
  // rho_g = 885.55 "Constant sodium density at 550 degC";
  cp_g = 1275.3 "Constant sodium specific heat capacity at 550 degC";
  k_g = 93.0 - 0.0581*T_g + 1.173e-5*T_g^2 "Liquid sodium thermal conductivity";
  // k_g = 62 "Constant gap conductivity at 550 degC";
  ...
  // cp_c = 1235.55*ones(radNodes_c) "Constant specific heat capacity at 550 degC";
  k_c = 17.622 .+ 2.42e-2 .* T_c .- 1.696e-5 .* T_c .^ 2;
  // k_c = 25*ones(radNodes_c);
  ...
  // k_c[radNodes_c]/(2*delta_c) * (T_cool - T_c[radNodes_c - 1]) + h * (T_c[radNodes_c] - T_cool) = 0;
  k_c[radNodes_c]/(2*delta_c)*(T_c[radNodes_c - 1] - T_c[radNodes_c - 2]) + h*(
    T_c[radNodes_c] - T_cool) = 0;
  wall.Q_flow + powerIn = 0;
  // Energy balance
...
Vector")}),
    ...
    Diagram(graphics));
end FuelPellet_wBoundaryConditions;
