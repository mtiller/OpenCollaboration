...
model CondPlant "Condensation Plant"
  extends SmAHTR.PowerConversionSystem.CondenserGroup.Interfaces.CondPlant;
  replaceable SmAHTR.PowerConversionSystem.Condensers.Condenser
    condenser(
    ...
    SmAHTR.PowerConversionSystem.CondenserGroup.Interfaces.Condenser(
    redeclare package FluidMedium = FluidMedium,
    N_cool=N_cool,
    condNomFlowRate=condNomFlowRate,
    coolNomFlowRate=coolNomFlowRate,
  ...
  ThermoPower3.Water.SourceW coolingIn(
    ...
    h=h_cool_in) annotation (Placement(transformation(extent={{90,-20},
            {70,0}}, rotation=0)));
  ThermoPower3.Water.SinkP coolingOut(redeclare package Medium =
        FluidMedium, p0=coolNomPressure) annotation (Placement(
        transformation(extent={{70,0},{90,20}}, rotation=0)));
  replaceable
    ThermoPower3.PowerPlants.SteamTurbineGroup.Components.BaseReader_water
    stateCoolingOut(redeclare package Medium = FluidMedium)
    constrainedby
    ThermoPower3.PowerPlants.SteamTurbineGroup.Components.BaseReader_water(
