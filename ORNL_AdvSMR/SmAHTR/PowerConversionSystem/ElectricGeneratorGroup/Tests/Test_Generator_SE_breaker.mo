...
model Test_Generator_SE_breaker
  ...
  ORNL_AdvSMR.Components.ValveVap valveHP(
    ...
    Cv=1165) annotation (Placement(transformation(
        ...
        extent={{6,6},{-6,-6}},
        rotation=90)));
  ORNL_AdvSMR.Components.SourceP sourceP(h=3.47e6, p0=1.296e7)
...
public
  Modelica.Blocks.Sources.Ramp com_valveHP_a(
    ...
    startTime=5,
    height=+0.7,
    duration=3) annotation (Placement(transformation(extent={{0,76},{
            -12,88}}, rotation=0)));
public
  Modelica.Blocks.Sources.Ramp com_valveHP_b(
    ...
    duration=5,
    height=-0.8,
    startTime=15) annotation (Placement(transformation(extent={{0,54},
            {-12,66}}, rotation=0)));
  Modelica.Blocks.Math.Add add annotation (Placement(transformation(
        origin={-49,69},
        extent={{-7,-7},{7,7}},
        rotation=180)));
  SmAHTR.PowerConversionSystem.ElectricGenerators.OldElementsSwingEquation.Generator_SE_Breaker
    generator_SE(
    Pmax=30e6,
    omega_nom=157.08,
    eta=0.96,
    ...
    omega_start=157.08,
    r=0.2,
    delta_start=0.5073) annotation (Placement(transformation(extent={
            {-30,-48},{50,32}}, rotation=0)));
  SmAHTR.PowerConversionSystem.ElectricGenerators.Grid grid(Pn=50e8,
      droop=0) annotation (Placement(transformation(extent={{66,-18},
            {86,2}}, rotation=0)));
public
...
equation
  ...
  connect(sourceP.flange, valveHP.inlet) annotation (Line(
      ...
      thickness=0.5,
      color={0,0,255}));
  connect(add.y, valveHP.theta) annotation (Line(points={{-56.7,69},{
          -62,69},{-62,24},{-71.2,24}}, color={0,0,127}));
  connect(com_valveHP_b.y, add.u1) annotation (Line(points={{-12.6,60},
          {-30,60},{-30,64},{-40.6,64},{-40.6,64.8}}, color={0,0,127}));
  connect(com_valveHP_a.y, add.u2) annotation (Line(points={{-12.6,82},
          {-30,82},{-30,74},{-40.6,74},{-40.6,73.2}}, color={0,0,127}));
  connect(generator_SE.shaft, steamTurbineStodola.shaft_b)
    annotation (Line(
      points={{-26,-8},{-53.76,-8}},
      color={0,0,0},
      thickness=0.5));
  connect(grid.connection, generator_SE.powerConnection) annotation (
      ...
      thickness=0.5));
  connect(booleanConstant.y, generator_SE.closed) annotation (Line(
        points={{59,56},{78,56},{78,30},{10,30},{10,19.2}}, color={
          255,0,255}));
