...
model TestDoubleShaft
  ...
  SmAHTR.PowerConversionSystem.SteamTurbines.SteamTurbineStodola
    ...
    eta_iso_nom=0.83,
    PRstart=5,
    pnom=12800000) annotation (Placement(transformation(extent={{-76,
            8},{-36,48}}, rotation=0)));
  ORNL_AdvSMR.Components.SourceW sourceW1(
    h=3.47e6,
    w0=67.6,
    p0=128e5) annotation (Placement(transformation(extent={{-96,54},{
            -82,68}}, rotation=0)));
...
public
  Modelica.Blocks.Sources.BooleanConstant booleanConstant(k=true)
    annotation (Placement(transformation(extent={{20,-70},{40,-50}},
          rotation=0)));
  SmAHTR.PowerConversionSystem.ElectricGeneratorGroup.Examples.DoubleShaft_static
    doubleShaft(
    Pn=50e8,
    J_shaft_A=2000,
  ...
  ORNL_AdvSMR.Components.SourceW sourceW2(
    w0=81.1,
    p0=26.8e5,
    h=3.554e6) annotation (Placement(transformation(extent={{-96,-6},
            {-82,8}}, rotation=0)));
...
equation
  ...
  connect(Turbine1.shaft_b, doubleShaft.shaft_A) annotation (Line(
      ...
      color={0,0,0},
      thickness=0.5));
  connect(actuators, doubleShaft.ActuatorsBus) annotation (Line(
        points={{74,-30},{92,-30},{92,-11},{80,-11}}, color={213,255,
          170}));
  connect(booleanConstant.y, actuators.breakerClosed_shaftA)
    annotation (Line(points={{41,-60},{60,-60},{60,-30},{74,-30}},
        color={255,0,255}));
  connect(booleanConstant.y, actuators.breakerClosed_shaftB)
    annotation (Line(points={{41,-60},{52,-60},{52,-30},{74,-30}},
        color={255,0,255}));
  connect(doubleShaft.SensorsBus, sensors) annotation (Line(points={{
          80,-2},{92,-2},{92,50},{72,50}}, color={255,170,213}));
