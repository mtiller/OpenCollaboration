...
model GeneratorGroup_wBreaker
  ...
  SmAHTR.PowerConversionSystem.ElectricGenerators.PowerSensor
    powerSensor annotation (Placement(transformation(extent={{20,-10},
            {40,10}}, rotation=0)));
  SmAHTR.PowerConversionSystem.ElectricGenerators.FrequencySensor
    frequencySensor annotation (Placement(transformation(extent={{20,
            40},{40,60}}, rotation=0)));
  SmAHTR.PowerConversionSystem.ElectricGenerators.Generator generator(
    ...
    initOpt=if SSInit then ThermoPower3.Choices.Init.Options.steadyState
      ...
      Placement(transformation(extent={{-100,-30},{-40,30}}, rotation=
           0)));
  SmAHTR.PowerConversionSystem.ElectricGenerators.NetworkGrid_Pmax
    network(
    hasBreaker=true,
    Pmax=Pmax,
    J=J_shaft,
  ...
  Modelica.Mechanics.Rotational.Components.Fixed fixed annotation (
      Placement(transformation(extent={{-50,-86},{-30,-66}}, rotation=
           0)));
  Modelica.Blocks.Interfaces.BooleanInput u annotation (Placement(
        transformation(extent={{220,30},{180,70}})));
equation
  connect(SensorsBus.power, powerSensor.W) annotation (Line(points={{
          200,-80},{30,-80},{30,-9.4}}, color={255,170,213}));
  ...
  connect(generator.shaft, shaft) annotation (Line(
      points={{-95.8,5.32907e-016},{-100,5.32907e-016},{-100,0},{-200,
          0}},
      color={0,0,0},
      thickness=0.5));
  ...
  connect(network.closed, u) annotation (Line(
      points={{100,19.4},{100,50},{200,50}},
      color={255,0,255},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false,
          extent={{-200,-200},{200,200}}), graphics));
end GeneratorGroup_wBreaker;
