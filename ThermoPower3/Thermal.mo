...
This package contains models to compute the material properties needed to model heat transfer and thermo-mechanical stresses in objects such as turbine shafts or headers.
  ...
  equation
    ...
    annotation (Diagram(graphics), Icon(graphics={Text(
              ...
              extent={{-10,-92},{96,-20}},
              lineColor={0,0,0},
              textString="HT"),Rectangle(extent={{-100,100},{100,-100}},
            lineColor={255,0,0}),Line(points={{100,100},{-100,-100}}, color={
            255,0,0})}));
  end HThtc_HT;
  ...
  equation
    ...
    annotation (Diagram(graphics), Icon(graphics={Text(
              ...
              extent={{-10,-92},{96,-20}},
              lineColor={0,0,0},
              textString="DHT"),Rectangle(extent={{-100,100},{100,-100}},
            lineColor={255,128,0}),Line(points={{100,100},{-100,-100}}, color={
            255,128,0})}));
  end DHThtc_DHT;
...
"),   Diagram(graphics));
  ...
  model ConvHT2N_htc
    ...
    parameter Integer N1(min=1) = 2 "Number of nodes on side 1";
    parameter Integer N2(min=1) = 2 "Number of nodes on side 2";
    ThermoPower3.Thermal.DHThtc_in side1(N=N1) annotation (Placement(
          transformation(extent={{-40,20},{40,40}}, rotation=0)));
    ThermoPower3.Thermal.DHT side2(N=N2) annotation (Placement(transformation(
...
<li><i>10 Jan 2004</i>
  ...
  model MetalTube "Cylindrical metal tube - 1 radial node and N axial nodes"
    ...
    parameter Temperature TstartN=Tstartbar
      ...
      annotation (Dialog(tab="Initialisation"));
    parameter Temperature Tstart[N]=ThermoPower3.Thermal.linspaceExt(
          Tstart1,
          TstartN,
...
"),   Diagram(graphics));
  ...
  model MetalWall "Generic metal wall - 1 radial node and N axial nodes"
    extends ThermoPower3.Icons.MetalWall;
    parameter Integer N(min=1) = 2 "Number of nodes";
    ...
    parameter ThermoPower3.Choices.Init.Options initOpt=ThermoPower3.Choices.Init.Options.noInit
      "Initialisation option" annotation (Dialog(tab="Initialisation"));
    constant Real pi=Modelica.Constants.pi;
    ThermoPower3.AbsoluteTemperature T[N](start=Tstart) "Node temperatures";
    ThermoPower3.Thermal.DHT int(N=N, T(start=Tstart)) "Internal surface"
      annotation (Placement(transformation(extent={{-40,20},{40,40}}, rotation=
              0)));
    ThermoPower3.Thermal.DHT ext(N=N, T(start=Tstart)) "External surface"
...
"),   Diagram(graphics));
  ...
  model CylinderFourier
    ...
    import Modelica.SIunits.*;
    import ThermoPower3.Choices.CylinderFourier.NodeDistribution;
    extends Icons.MetalWall;
    ...
    parameter NodeDistribution nodeDistribution=ThermoPower3.Choices.CylinderFourier.NodeDistribution.uniform
      "Node distribution";
  ...
  public
    ...
    ThermoPower3.Thermal.DHT internalBoundary(final N=1) annotation (Placement(
          transformation(extent={{-20,20},{20,40}}, rotation=0)));
    ThermoPower3.Thermal.DHT externalBoundary(final N=1) annotation (Placement(
